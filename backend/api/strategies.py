...
from models.database import StrategyConfig, get_db
...
async def list_strategies(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(StrategyConfig).order_by(StrategyConfig.is_builtin.desc()))
...
async def create_strategy(body: StrategyCreate, db: AsyncSession = Depends(get_db)):
    ...
    strategy = StrategyConfig(name=name, yaml_content=body.yaml_content, is_builtin=False)
    db.add(strategy)
    await db.commit()
...
async def update_strategy(strategy_id: int, body: StrategyUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if strategy.enabled:
        await reschedule(parsed.get("schedule", "*/30 * * * *"))
...
async def enable_strategy(strategy_id: int, db: AsyncSession = Depends(get_db)):
    """Set this strategy as the active one (disables all others)."""
    result = await db.execute(select(StrategyConfig).where(StrategyConfig.id == strategy_id))
    strategy = result.scalar_one_or_none()
    if strategy is None:
        raise HTTPException(404, "Strategy not found")
    ...
    # Disable all
    all_result = await db.execute(select(StrategyConfig))
...
async def disable_strategy(strategy_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(StrategyConfig).where(StrategyConfig.id == strategy_id))
    strategy = result.scalar_one_or_none()
    if strategy is None:
        raise HTTPException(404, "Strategy not found")
